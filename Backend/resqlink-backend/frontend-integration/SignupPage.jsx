// src/Pages/SignupPage.jsx  ── UPDATED: wired to real backend
import { useState, useEffect, useRef } from "react";
import { Link, useNavigate }           from "react-router-dom";
import { useUser }                     from "../Components/usercontext";
import { authAPI }                     from "../services/api";

const CheckIcon  = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>);
const ArrowRight = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14m-7-7 7 7-7 7" /></svg>);
...
export default function SignupPage() {
  ...
  // Stored after step-2 API call
  const [registrationMeta, setRegistrationMeta] = useState({ userId: null, phone: null });
  ...
  const handleStep2Continue = async () => {
    ...
    setApiError("");
    try {
      const { data } = await authAPI.signup(formData, selectedRole);
      setRegistrationMeta({ userId: data.userId, phone: data.phone });
      // In dev the OTP is returned; auto-fill it for convenience
      if (data.otp) {
  ...
  const handleResendOtp = async () => {
    ...
    setApiError("");
    try {
      const { data } = await authAPI.resendOtp(registrationMeta.userId);
      if (data?.otp) setOtp(String(data.otp).split(""));
      setTimer(32);
    } catch (err) {
      setApiError(err.message || "Could not resend OTP.");
    }
  };

  // ── Final submit — verify OTP ──────────────────────
  const handleSubmit = async () => {
    ...
    setApiError("");
    try {
      const { data } = await authAPI.verifyOtp(registrationMeta.userId, otpStr);
      login(data.user);
    ...
    <>
      ...
      <div className="main_container">
        ...
        <div className="resq-main">
          ...
          {apiError && (
            <div style={{ marginBottom: "1rem", padding: "12px 16px", borderRadius: 10, background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.3)", color: "#ef4444", fontSize: 13, textAlign: "center" }}>
              {apiError}
            </div>
          )}
          ...
          <div className="resq-card">
            ...
            {step === 2 && (
              <div>
                ...
                <div className="field-grid">
                  ...
                  {selectedRole === "ngo" && (
                    <>
                      <label>
                        <span className="field-label">Organization Name</span>
                        <input className="field-input" name="orgName" placeholder="Red Cross Base" value={formData.orgName} onChange={handleInputChange} />
                        {errors.orgName && <span className="error-txt">{errors.orgName}</span>}
                      </label>
                      <label>
                  ...
                  {selectedRole === "volunteer" && (
                    <>
                      ...
                      <div className="full">
                        <span className="field-label" style={{ marginBottom: ".5rem" }}>Availability</span>
                ...
                <div className="btn-row">
                  ...
                  <button type="button" className="btn btn-primary" onClick={handleStep2Continue} disabled={isSubmitting}>
                    {isSubmitting ? "Creating account…" : <><span>Continue</span> <ArrowRight /></>}
                  </button>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div>
                ...
                <p>We sent a 6-digit code to <strong>{registrationMeta.phone || formData.phone}</strong>.</p>
                ...
                <p className="otp-note">
                  {timer > 0
                    ...
                    : <button type="button" className="resq-resend" onClick={handleResendOtp}>Resend OTP</button>}
                </p>
                <div className="btn-row">
                  <button type="button" className="btn btn-outline" onClick={() => setStep(2)} disabled={isSubmitting}>Back</button>
                  <button type="button" className="btn btn-primary" onClick={handleSubmit} disabled={isSubmitting}>
                    {isSubmitting ? "Verifying…" : <><span>Verify &amp; Enter</span> <ArrowRight /></>}
                  </button>
                </div>
              </div>
            )}
...
const styles = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .resq-step-node { width: 2rem; height: 2rem; border-radius: 50%; display: grid; place-items: center; font-size: .75rem; font-weight: 600; transition: all 0.3s; }
  ...
  .btn { display: inline-flex; align-items: center; justify-content: center; gap: .5rem; height: 2.75rem; padding: 0 1.5rem; border-radius: .5rem; font-size: .9rem; font-weight: 600; cursor: pointer; border: none; transition: all 0.2s; }
  ...
  .btn-outline:hover:not(:disabled) { background: #212124; }
  ...
  .resq-resend { background: none; border: none; color: #ef4444; font-weight: 600; cursor: pointer; text-decoration: underline; font-size: .875rem; }
`;
